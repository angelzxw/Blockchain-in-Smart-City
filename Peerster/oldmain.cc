...
{
	return hashPartialBlock(block.name, block.index, block.time, block.prev_hash, block.rand).startsWith(QString("00000").toAscii());
}

inline bool verifyHash(QByteArray hash)
{
	return hash.startsWith(QString("00000").toAscii());
}
...
*/
  ...
  // 	}
  	...
  	NewBlockThread* nBT = new NewBlockThread();
  	nBT->start();
...
void ChatDialog::sendNewBLOCK(Block block, const QHostAddress addr, quint16 port)
{
	QVariantMap dataMap;
	QByteArray dataArray;
	QDataStream out(&dataArray,QIODevice::WriteOnly);
...
void ChatDialog::sendNewBLOCKToNeighbors(Block block, QHostAddress exSender = QHostAddress::Null, quint16 exSenderPort = 0)
{
	...
	for(quint16 i = 1; i < peerList.size(); i++)
	{
		...
		{	
			sendNewBLOCK(block, peer->IPAddress, peer->UDPPortNum);
		}
	}
}

void ChatDialog::sendWantBLOCKMsg(const QHostAddress addr, quint16 port, quint16 seq)
{
	QVariantMap dataMap;
	QByteArray dataArray;
	QDataStream out(&dataArray,QIODevice::WriteOnly);
...
void ChatDialog::sendInquiry()
{
	Peer* p =getRandNeighbor();
  	...
  	sendWantBLOCKMsg(p->IPAddress, p->UDPPortNum, blockchain.size() + 1);
...
{
	QByteArray hash = hashBlock("Genesis", 1, QTime(), QByteArray(), QMap<QString, QVariant>(), 602813);
	Block* genesis = new Block("Genesis", 1, QTime(), QByteArray(), QMap<QString, QVariant>(), hash, 602813);
	return genesis;
}

Block* ChatDialog::createNewBlock(QString name, quint32 index, QTime time, QByteArray prev_hash, QVariantMap data)
{
	quint64 i = qrand();
	while(true)
	{
...
*/
    if(dataMap.contains("Hash"))
    {
    	...
    	if(index == blockchain.size() + 1 && prev_hash == blockchain.last()->hash &&
    		verifyHash(hashPartialBlock(name, index, time, prev_hash, rand)) && hash == hashBlock(name, index, time, prev_hash, data, rand))
    	{
    		...
    		sendNewBLOCKToNeighbors(*newBlock, sender, senderPort);
    	}
    	else
    	{	
    ...
    else if(dataMap.contains("WantBLOCK"))
    {
		...
		if(diff > 1)
		{
			sendWantBLOCKMsg(sender, senderPort, blockchain.size() + 1);
		}
		else if(diff < 1)
		{
			sendNewBLOCK(*(blockchain.at(seq - 1)), sender, senderPort);
		}
    }
